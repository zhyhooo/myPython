...
def print_current_indicators(df, stock_code):
    ...
    print(f"   ** 多头排列：MA5 > MA10 > MA20 > MA60 → 上升趋势")
    print(f"   ** 空头排列：MA5 < MA10 < MA20 < MA60 → 下跌趋势")
    print(f"   ** 金叉：短期均线上穿长期均线 → 买入信号")
    print(f"   ** 死叉：短期均线下穿长期均线 → 卖出信号")

    print(f"\n🎯 趋势指标:")
    print(f"   MA5: {current['MA5']:.2f} | MA20: {current['MA20']:.2f} | MA60: {current['MA60']:.2f}")
    print(f"   布林带位置(Bollinger Bands): [{current['BB_Lower']:.2f}, {current['BB_Upper']:.2f}]")
    print(f"   ** 中轨：20日移动平均线，上/下轨: 中轨 ± 2倍标准差")
    print(f"   ** 价格在上/下轨附近：可能超买/卖")
    print(f"   ** 带宽收窄：波动率降低，可能即将突破")


    print(f"\n⚡ 动量指标:")
    print(f"   MACD: {current['MACD']:.4f} | DIF: {current['DIF']:.4f} | DEA: {current['DEA']:.4f}")
    print(f"   RSI14: {current['RSI14']:.1f} | RSI6: {current['RSI6']:.1f}")
    print(f"   ** 30(超卖区，可能反弹) < RSI14 < 70(超买区，可能回调)")
